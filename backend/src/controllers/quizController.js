    ...
    jwt.verify(
      ...
      async (err, data) => {
        ...
        try {
          ...
          let updatedValue = await Quiz.updateOne(
            { _id: id },
...
function pickQuestionsRandomly(wordList) {
  ...
  Object.keys(selectedWord).map((id) => {
    ...
    while (options.length != 4) {
      ...
      const hasAlreadySelected = options?.find(
        (item) => item?.id === meaningList[index]._id
      );

      if (hasAlreadySelected) continue;
